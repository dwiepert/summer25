...
def test_dataset_initialization():
    ...
    d = WavDataset(data=df, prefix='./tests/audio_examples/', model_type='wavlm-base', config=config, target_labels=[_FEATURES[0]], bucket=None, feature_extractor=None, transforms=transform)
...
def test_get_item():
    ...
    # AUDIO ONLY
    d = WavDataset(data=df, prefix='./tests/audio_examples/', model_type='whisper-tiny', config=config, target_labels=[_FEATURES[0]], bucket=None, feature_extractor=None, extension='flac')
    out = d[0]
    ...
    d2 = WavDataset(data=df, prefix='./tests/audio_examples/', model_type='whisper-tiny', config=config2, target_labels=[_FEATURES[0]], bucket=None, feature_extractor=None, extension='flac')
    out = d2[0]
    assert all([u in out for u in ['uid','targets','waveform','sample_rate']])
    assert isinstance(out['uid'],str)
    assert isinstance(out['targets'],torch.Tensor)
    ...
    d3 = WavDataset(data=df, prefix='./tests/audio_examples/', model_type='whisper-tiny', config=config3, target_labels=[_FEATURES[0]], bucket=None, feature_extractor=None, extension='flac')
    out = d3[0]
    assert all([u in out for u in ['uid','targets','waveform','sample_rate']])
    assert isinstance(out['uid'],str)
    assert isinstance(out['targets'],torch.Tensor)
    ...
    d4 = WavDataset(data=df, prefix='./tests/audio_examples/', model_type='whisper-tiny', config=config4, target_labels=[_FEATURES[0]], bucket=None, feature_extractor=None, extension='flac')
    out = d4[0]
    assert all([u in out for u in ['uid','targets','waveform','sample_rate']])
    assert isinstance(out['uid'],str)
    assert isinstance(out['targets'],torch.Tensor)
    ...
    d5 = WavDataset(data=df, prefix='./tests/audio_examples/', model_type='wavlm-base', config=config4, target_labels=[_FEATURES[0]], bucket=None, feature_extractor=None, extension='flac')
    out = d5[0]
    assert all([u in out for u in ['uid','targets','waveform','sample_rate']])
    assert isinstance(out['uid'],str)
    assert isinstance(out['targets'],torch.Tensor)
...
def test_dataset_feature_extractor():
    ...
    d = WavDataset(data=df, prefix='./tests/audio_examples/', model_type='whisper-tiny', config=config, target_labels=[_FEATURES[0]], bucket=None, feature_extractor=feature_extractor, extension='flac')
    out = d[0]
    assert all([u in out for u in ['uid','targets','waveform','sample_rate']])
    assert isinstance(out['uid'],str)
    assert isinstance(out['targets'],torch.Tensor)
